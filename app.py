...
app = Flask(__name__)
app.secret_key = "athleticore_super_secure_vault_key_2026"
...
DATABASE = {
    ...
    "ball_sports": {
        "football": [
            {"id": "fb_1", "home": "Mater Dei", "away": "St. John Bosco", "score": "28 - 24", "status": "LIVE - Q4 2:15", "play": "Mater Dei completes a 14-yard pass for a first down to the Bosco 20 yard line."},
...
def index():
    ...
    if "username" not in session:
        session["username"] = "Guest_Athlete"
...
def auth_api():
    ...
    session["username"] = f"{provider}_User_734"
    return jsonify({"status": "success", "username": session["username"], "wallet": session["wallet"]})
...
@app.route("/api/wager", methods=["POST"])
def wager_api():
    data = request.get_json() or {}
    amount = int(data.get("amount", 0))
    
    if amount <= 0 or amount > session.get("wallet", 0):
        return jsonify({"status": "error", "message": "Invalid funds allocation."})
    ...
    if won:
        ...
        msg = f"Prediction successful! Credited +{winnings} RUN tokens to ledger."
    else:
        msg = "Match resolved against your prediction parameters."
        
    return jsonify({"status": "success", "won": won, "message": msg, "wallet": session["wallet"]})
...
def market_api():
    ...
    if session.get("wallet", 0) < cost:
        return jsonify({"status": "error", "message": "Insufficient balance inside wallet ledger."})
        
    session["wallet"] -= cost
    if item_name not in session["badges"]:
        session["badges"].append(item_name)
        
    return jsonify({"status": "success", "wallet": session["wallet"], "badges": session["badges"]})

@app.route("/api/live-stream")
def live_stream_api():
    for item in DATABASE["ball_sports"]["football"]:
        h, a = map(int, item["score"].split(" - "))
        if random.random() > 0.7: h += random.choice([3, 6, 7])
        if random.random() > 0.7: a += random.choice([3, 6, 7])
        item["score"] = f"{h} - {a}"
        item["play"] = random.choice([
            "Quarterback sacked in the backfield for a severe 7-yard loss!",
            "Field goal attempt from 42 yards out is split perfectly through the uprights.",
            "Interception! Cornerback jumps the quick route at the 45-yard line!",
            "Incomplete pass forcing a tough 4th down punt sequence upcoming."
        ])
        
    for item in DATABASE["ball_sports"]["basketball"]:
        h, a = map(int, item["score"].split(" - "))
        if random.random() > 0.4: h += random.choice([2, 3])
        if random.random() > 0.4: a += random.choice([2, 3])
        item["score"] = f"{h} - {a}"
        item["play"] = random.choice([
            "Slam dunk down the middle lane off a perfect fastbreak alley-oop pass!",
            "Shooting foul called down low. Going to the charity stripe for two shots.",
            "Time-out called by the head coach to draw up a tactical baseline set play.",
            "Steal! Guard swipes the cross-court pass and drives coast to coast."
        ])
